...
# Function to process the mesh and create offset with cylinders and spheres
def process_mesh_and_create_offset():
    ...
    # Offset downward by 200mm (0.2m)
    offset_lowest = (lowest_point[0], lowest_point[1], lowest_point[2] - 0.2)
    offset_second = (second_point[0], second_point[1], second_point[2] - 0.2)
    ...
    # 4. Process both Meshes: QuadRemesh, extract points, and create geometry
    # QuadRemesh the original Mesh and update mesh_id
    rs.Command("-QuadRemesh TargetQuadCount 21 AdaptiveSize 50 _SelID {} _Enter".format(original_mesh_id))
    ...
    mesh_id = quad_mesh_id  # Update mesh_id to the QuadRemeshed version
    
    # QuadRemesh the new horizontal Mesh and update new_mesh_id
    ...
    # Process the two final Meshes
    meshes_to_process = [mesh_id, new_mesh_id]
    all_points = []
    ...
    for idx, m_id in enumerate(meshes_to_process):
        ...
        print("Mesh {} - Lowest point: ({:.2f}, {:.2f}, {:.2f})".format(idx, lowest[0], lowest[1], lowest[2]))
        ...
        for pt in points:
            ...
            if idx == 1:  # Horizontal Mesh (new_mesh_id)
    ...
    # 5. Connect corresponding points between the two Meshes
    if len(all_points) == 2:
        min_points = min(len(all_points[0]), len(all_points[1]))
        print("Connecting {} points between Meshes".format(min_points))
        for i in range(min_points):
            start = all_points[0][i]
            end = all_points[1][i]
            if start != end:
                line = rs.AddLine(start, end)
                if not line:
                    print("Failed to add line between points {} and {}".format(start, end))
            else:
                print("Skipped connecting identical points: {}".format(start))
    else:
        print("Cannot connect points: Not enough Meshes processed!")
    
    # Label the working plane
    rs.AddText("Working Plane: Z = {:.2f}".format(offset_lowest[2]), offset_lowest, height=0.1)

    # Delete the original unprocessed Mesh to clean up
    rs.DeleteObject(original_mesh_id)
